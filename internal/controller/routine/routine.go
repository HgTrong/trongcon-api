...
func NewController(svc service.RoutineService) *Controller {
	return &Controller{svc: svc}
}

func (c *Controller) Create(ctx *gin.Context) {
	var req routinev1.CreateReq
...
func (c *Controller) GetByIDPublic(ctx *gin.Context) {
	...
	res, err := c.svc.GetByIDPublic(ctx.Request.Context(), id, viewerUserID)
	if err != nil {
		writeErr(ctx, err)
		return
	}
...
func (c *Controller) Delete(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, routinev1.DeleteRes{Status: "ok"})
}
