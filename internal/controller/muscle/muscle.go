...
type Controller struct {
	svc service.MuscleService
}

func NewController(svc service.MuscleService) *Controller {
	return &Controller{svc: svc}
}

func (c *Controller) Create(ctx *gin.Context) {
	var req musclev1.CreateReq
...
func (c *Controller) Delete(ctx *gin.Context) {
	...
	ctx.JSON(http.StatusOK, musclev1.DeleteRes{Status: "ok"})
}
...
func writeErr(ctx *gin.Context, err error) {
	switch {
	case errors.Is(err, service.ErrMuscleNotFound):
